...
class CooperativeProactive(Strategy):
    def __init__(self, player):
        super().__init__("proactive_coop", player)

    def play(self, selectable, round):
        if round == 1:
            if len(self.player.table) == 3 and self.player.table[1] == max([card for card in self.player.table
                                                                            if card[1] == self.player.table[0][1]]) \
                    and selectable[0][1] == self.player.table[0][1]:
                chosen = max(selectable, key=lambda x: (x[0], -len([card for card in selectable if card[1] == x[1]])))

            elif not self.player.table:
                if [card for card in selectable if card[1] in self.player.partner] != [] and len(self.player.hand) > 3:
                    chosen = min([card for card in selectable if card[1] in self.player.partner],
                    ...
                    while i <= len(occurrences) - 1 and not selected:
                        if occurrences[i][0] > 0:
                            ...
                            while k <= len(aux) - 1:
                                l = 0
                                count = 0
                                while l <= len(aux3) - 1 and aux3[l][0] < aux[k][
                                    0]:  # check if it is in the 3 smallest cards of the suit 
                                    count += 1
                                    l = l + 1
                                if count < 3 and aux3 != [] and count != len(
                                        aux3):
                                    chosen = aux[k]
                                    selected = True
                                k = k + 1
            ...
            if self.player.table:
                ...
                if selectable[0][1] == drown[1]:  # selectable contains one card of the same suit at least
                    ...
                    while j <= len(selectable) - 1:
                        ...
                        j = j + 1
                    if lst1 != []:
                        chosen = lst1[-1]  # chosen is the closest undervalued card of the 1st card played
                    else:
                        if drown[1] == 'D':
                            aux2 = self.player.diamonds
                        elif drown[1] == 'H':
                    ...
                    while i <= len(occurrences) - 1:
                        if occurrences[i][0] > 0:
                            ...
                            while j <= len(selectable) - 1:
                                ...
                                j = j + 1
                            if aux2[
                               :len(aux)] == aux:  # Are they the lowest cards in the deck?
                                if i == 3:
            ...
            if self.player.table != []:
                ...
                if selectable[0][1] == drown[1]:  # selectable contains one card of the same suit at least
                    ...
                    if len(aux3) > 5:
                        ...
                        if len(self.player.table) == 3:  # see the case of copas in the tables ... 
                            chosen = max(selectable)
                        elif aux3:
                    ...
                    while i <= len(occurrences) - 1 and not selected:
                        ...
                        if occurrences[i][0] == 1 and occurrences[i][1] != 'H':
                            ...
                            if [card for card in selectable if card[1] != 'H'] != [] \
                                    and ((len(self.player.table) == 2 and
                                          self.player.table[0] == max([card
                                                                       for card in self.player.table
                                                                       if card[1] == self.player.table[0][1]]))
                                         or (len(self.player.table) == 3 and
                                             self.player.table[1] == max([card for card in self.player.table
                                                                          if card[1] == self.player.table[0][1]]))):
                                if occurrences[i][0] > 0:
                                    if occurrences[i][1] == 'S':
                                    ...
                                    if occurrences[i][1] == 'C':
                                        ...
                                        selected = True
                            elif self.player.hearts[:len(aux)] != aux:  # Do we have the lowest hearts yet to play?
                                chosen = max(aux)
                                selected = True
                            elif self.player.hearts[:len(aux)] == aux and occurrences[3][1] == 'H' and occurrences[3][
                                0] > 0:
                                chosen = max(aux)
            ...
            if not self.player.table:
                ...
                if selectable[0][1] == drown[1]:  # selectable contains one card of the same suit at least
                    ...
                    if dama and dama_card[1] == drown[1] and \
                            ((len(self.player.table) == 2 and
                              self.player.table[0] ==
                              max([card for card in self.player.table if card[1] == self.player.table[0][1]])) or
                             (len(self.player.table) == 3 and
                              self.player.table[1] ==
                              max([card for card in self.player.table if card[1] == self.player.table[0][1]]))):
                        chosen = max(selectable)
                    ...
                    while i <= len(self.player.table) - 1:
                        ...
                        i += 1
                    if over12:
                        if ((len(self.player.table) == 2 and
                             self.player.table[0] == max([card for card in self.player.table
                                                          if card[1] == self.player.table[0][1]])) or
                                (len(self.player.table) == 3 and
                                 self.player.table[1] == max([card for card in self.player.table
                                                              if card[1] == self.player.table[0][1]]))):
                            ...
                            if dama:
                                ...
                                while k <= len(aux) - 1:
                                    if aux[k][0] < 12:
                                        under12 += [aux[k]]
                                    k += 1
                                if len(under12) < len(aux3) and \
                                        ((len(self.player.table) == 2 and self.player.table[0] ==
                                          max([card for card in self.player.table if card[1] ==
                                                                                     self.player.table[0][1]])) or
                                         (len(self.player.table) == 3 and self.player.table[1] ==
                                          max([card for card in self.player.table if card[1] ==
                                                                                     self.player.table[0][1]]))) and \
                                        [item for item in selectable if item[0] != 12]:
                                    chosen = max([item for item in selectable if item[0] != 12])
                                    selected = True

                                elif ((len(self.player.table) == 2 and self.player.table[0] == max(
                                        [card for card in self.player.table if
                                         card[1] == self.player.table[0][1]])) or (
                                              len(self.player.table) == 3 and self.player.table[1] == max(
                                          [card for card in self.player.table if card[1] == self.player.table[0][1]]))):
                                    if max(selectable)[0] == 12:
                                        if len(selectable) == 1:
                                            chosen = max(selectable)
                                            selected = True
            ...
            if not self.player.table:
                ...
                if selectable[0][1] == drown[1]:  # selectable contains one card of the same suit at least
                    ...
                    if men:
                        ...
                        if card == (11, selectable[0][1]) and len(self.player.table) == 3 and (
                                (len(self.player.table) == 2 and self.player.table[
                                    0] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[
                                    1] == max(
                                    [card for card in self.player.table if
                                     card[1] == self.player.table[0][1]]))):
                            chosen = max(selectable)
                        selectable2 = []
                        if card == (11, selectable[0][1]) and len(self.player.table) == 2 and (
                                (len(self.player.table) == 2 and self.player.table[
                                    0] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[
                        ...
                        if card == (13, selectable[0][1]) and len(self.player.table) == 3 and (
                                (len(self.player.table) == 2 and self.player.table[
                                    0] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[
                                    ...
                                    [card for card in self.player.table if
                                     card[1] == self.player.table[0][1]]))):
                            chosen = max(selectable)
                        if card == (13, selectable[0][1]) and len(self.player.table) == 2 and (
                                (len(self.player.table) == 2 and self.player.table[
                                    0] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[
                    ...
                    while i <= len(self.player.table) - 1:
                        if self.player.table[i][1] == drown[1] and (
                                self.player.table[i][0] == 12 or self.player.table[i][0] == 13
                                or self.player.table[i][0] == 14):
                            above = True
                    ...
                    if above:
                        ...
                        if card == (12, selectable[0][1]) and len(self.player.table) == 3 and (
                                (len(self.player.table) == 2 and self.player.table[
                                    0] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[1] == max(
                            ...
                            chosen = max(selectable2)
                        if card == (12, selectable[0][1]) and len(self.player.table) == 2 and (
                                (len(self.player.table) == 2 and self.player.table[
                                    0] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[
                        ...
                        if card == (13, selectable[0][1]) and len(self.player.table) == 2 and (
                            ...
                            chosen = max(selectable)
                        if card == (14, selectable[0][1]) and len(self.player.table) == 3 and (
                                (len(self.player.table) == 2 and self.player.table[
                                    0] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[
                                    ...
                                    [card for card in self.player.table if
                                     card[1] == self.player.table[0][1]]))):
                            chosen = max(selectable)
                        if card == (14, selectable[0][1]) and len(self.player.table) == 2 and (
                                (len(self.player.table) == 2 and self.player.table[
                                    0] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[
                    ...
                    while i <= len(occurrences) - 1 and not selected:
                        ...
                        if selected and ((len(self.player.table) == 2 and self.player.table[0] == max(
                                [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                                 len(self.player.table) == 3 and self.player.table[1] == max(
                                             [card for card in self.player.table if
                                              card[1] == self.player.table[0][1]]))):
                        ...
                        if occurrences[i][0] > 0:
                            ...
                            while j <= len(aux) - 1:
                                if aux[j][0] > 11:
                                    over11 += [aux[j]]
                                j += 1
                            if over11:  # selected and
                                if ((len(self.player.table) == 2 and self.player.table[0] == max(
                                        [card for card in self.player.table if
                                         card[1] == self.player.table[0][1]])) or (
                                        len(self.player.table) == 3 and self.player.table[1] == max(
                                    [card for card in self.player.table if card[1] == self.player.table[0][1]]))):
                    ...
                    while i <= len(occurrences) - 1 and not selected:
                        ...
                        if occurrences[i][0] > 0:
                            ...
                            if aux4[:len(aux)] == aux:  # and not selected:
                                chosen = max(aux)
                                selected = True
                            else:
                                chosen = min(aux)
            ...
            if (13, 'H') in selectable and ((len(self.player.table) == 2 and self.player.table[0] == max(
                    [card for card in self.player.table if card[1] == self.player.table[0][1]])) or (
                                                    len(self.player.table) == 3 and self.player.table[1] == max(
                                                [card for card in self.player.table if
                                                 card[1] == self.player.table[0][1]]))):
                if self.player.table[0][1] != 'H':
                    chosen = min(self.player.hand)
                else:
                    chosen = min([card for card in self.player.hand if card[1] == 'H'])
                self.player.hand.remove(chosen)
            else:
                if len(self.player.table) == 1:
                    drown = self.player.table[0]  # 1st card played in the table
                    ...
                    if not selected:
                        ...
                        while i <= len(occurrences) - 1 and not selected:
                            if occurrences[i][0] > 0:
                                ...
                                if occurrences[i][1] == 'H':
                                    aux2 = self.player.hearts
                                    aux3 = [x for x in self.player.hearts if
                                            x not in selectable]
                                if occurrences[i][1] == 'C':
                                    aux2 = self.player.clubs
                                    aux3 = [x for x in self.player.clubs if x not in selectable]
                                if occurrences[i][1] == 'S':
                                    aux2 = self.player.spades
                                    aux3 = [x for x in self.player.spades if
                                            x not in selectable]
                                if len(aux3) <= 5:
                                    chosen = min(selectable)
                                    selected = True
            ...
            if not self.player.table:
                occurrences = []  # number of cards in hand for each suit
                nd, nh, nc, ns = 0, 0, 0, 0
                j = 0
                while j <= len(self.player.hand) - 1:
                    ...
                    j += 1
                occurrences = [[nd, 'D'], [nh, 'H'], [nc, 'C'], [ns, 'S']]  # list of occurrences for each suit 
                if [card for card in selectable if card[1] in self.player.partner]:
                    dry = []
                    if 'D' in self.player.partner and [card for card in selectable if card[1] == 'D'] != []:
                        dry += [occurrences[0]]
                    if 'H' in self.player.partner and [card for card in selectable if card[1] == 'H'] != []:
                        dry += [occurrences[1]]
                    if 'C' in self.player.partner and [card for card in selectable if card[1] == 'C'] != []:
                        dry += [occurrences[2]]
                    if 'S' in self.player.partner and [card for card in selectable if card[1] == 'S'] != []:
                        dry += [occurrences[3]]
                    occurrences = sorted(dry, key=lambda x: -x[0]) + sorted([x for x in occurrences if x not in dry],
                                                                            key=lambda x: -x[0])
                else:
                    occurrences = sorted(occurrences, key=lambda x: -x[0])
            ...
            if self.player.table:
                ...
                if selectable[0][1] == drown[1]:  # selectable contains one card of the same suit at least
                    ...
                    if len(self.player.hand) > 3:
                        ...
                        chosen = min(selectable) if [card for card in selectable if card[0] < drown[0]] == [] else max(
                            [card for card in selectable if card[0] < drown[0]])
                else:
                    i = 0
                    drown = self.player.table[0]
                    while i <= len(self.player.table) - 1:
