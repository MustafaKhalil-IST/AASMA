...
class CooperativeReactive(Strategy):
    def __init__(self, player):
        super().__init__("reactive_coop", player)
    ...
    def play(self, selectable, round):
        if round == 1:
            if len(self.player.table) == 1:
                ...
                if not [card for card in self.player.hand if card[1] == follow]:
                    ...
                    # if the agent must follow suit, it will play the highest card that is lower than what is on
                    # the table; if all of its cards are higher, it plays the highest card
                ...
                # if the agent is leading, it will play its lowest card; in case of a tie, it plays from the suit
                # that has fewer cards
                ...
                highest = max([card for card in self.player.table if card[1] == self.player.table[0][1]])
                partner = self.player.table[-2]  # card played by the partner
                if highest == partner or [card for card in self.player.hand if card[1] == follow] == []:
        ...
        if round == 2:
            ...
            if len(self.player.table) == 1:
                if not [card for card in self.player.hand if card[1] == follow]:
                    # if the agent cannot follow suit, it plays the highest heart; if it does not have any, it plays
                    # the highest card
                    ...
                    # if the agent must follow suit, it will play the highest card that is lower than what is on
                    # the table; if all of its cards are higher, it plays the highest card; exception: if the suit
                    # is hearts, it will always play the lowest card
                    if follow == 'H':
                ...
                # if the agent is leading, it will play its highest card in the first 3 rounds and the lowest otherwise.
                ...
                if highest == partner:
                    if [card for card in selectable if card[1] != 'H']:
                        chosen = max([card for card in selectable if card[1] != 'H'],
                                     key=lambda x: (x[0], -len([card for card in selectable if card[1] == x[1]])))
                    else:
                        chosen = max(selectable)
                else:
                    if not [card for card in self.player.hand if card[1] == follow]:
                        # if the agent cannot follow suit, it plays the highest heart; if it does not have any,
                        ...
                        if follow == 'H':
                            ...
                            max_table = max([card[0]
                                             for card in self.player.table if card[1] == self.player.table[0][1]])
                            min_sel = min([card[0] for card in selectable])
                            if min_sel < max_table:
                                chosen = max([card for card in selectable if card[0] < max_table], key=lambda x: x[0])
                            else:
        ...
        if round == 3:
            ...
            if len(self.player.table) == 1:
                if not [card for card in self.player.hand if card[1] == follow]:
                    # if the agent cannot follow suit, it plays a Queen; if it does not have one, it plays
                    # the highest card
                    ...
                    # if the agent must follow suit, it will play the highest card up to Jack, unless the highest
                    # card on the table is higher than Jack
                    max_table = max([card[0] for card in self.player.table if card[1] == self.player.table[0][1]])
                    if max_table >= 12:
                ...
                if highest == partner:
                    if [card for card in selectable if card[0] != 12]:
                        chosen = max([card for card in selectable if card[0] != 12],
                                     key=lambda x: (x[0], -len([card for card in selectable if card[1] == x[1]])))
                    else:
                    ...
                    if not [card for card in self.player.hand if card[1] == follow]:
                        ...
                        # if the agent must follow suit, it will play the highest card up to Jack,
                        # unless the highest card on the table is higher than Jack
                        max_table = max([card[0] for card in self.player.table if card[1] == self.player.table[0][1]])
                        if max_table >= 12:
                            if (12, follow) in selectable:
                                chosen = (12, follow)
        ...
        if round == 4:
            ...
            if len(self.player.table) == 1:
                if not [card for card in self.player.hand if card[1] == follow]:
                    # if the agent cannot follow suit, it plays a King or Jack; if it does not have one,
                    # it plays the highest card
                ...
                # if the agent is leading, it will play its highest card up to 10; in case of a tie, it plays from
                # the suit that has fewer cards
                ...
                if highest == partner:
                    if [card for card in selectable if card[0] != 11 and card[0] != 13]:
                        chosen = max([card for card in selectable if card[0] != 11 and card[0] != 13],
                                     key=lambda x: (x[0], -len([card for card in selectable if card[1] == x[1]])))
                    else:
                        chosen = max(selectable,
                                     key=lambda x: (x[0], -len([card for card in selectable if card[1] == x[1]])))
                    ...
                    if not [card for card in self.player.hand if card[1] == follow]:
                        ...
                        # if the agent must follow suit, it will play the highest card up to 10, unless the highest
                        # card on the table is higher than 10
                        max_table = max([card[0] for card in self.player.table if card[1] == self.player.table[0][1]])
                        if max_table == 14 and (13, follow) in selectable:
                            chosen = (13, follow)
                        elif max_table >= 12 and (11, follow) in selectable:
        ...
        if round == 5:
            if len(self.player.table) == 1:
                ...
                highest = max([card for card in self.player.table if card[1] == self.player.table[0][1]])
                partner = self.player.table[-2]  # card played by the partner
                if highest == partner and (13, 'H') not in self.player.table:
                    chosen = max(selectable,
                                 key=lambda x: (x[0], -len([card for card in selectable if card[1] == x[1]])))
                else:
                    follow = self.player.table[0][1]
                    if not [card for card in self.player.hand if card[1] == follow]:
                        ...
                        # if the agent must follow suit, it will play the highest card that is lower than what is on
                        # the table, except if that suit is hearts
                        if follow == 'H':
                            chosen = min(selectable, key=lambda x: x[0])
                        else:
                            max_table = max(
                                [card[0] for card in self.player.table if card[1] == self.player.table[0][1]])
                            if [card for card in selectable if card[0] < max_table]:
                                chosen = max([card for card in selectable if card[0] < max_table], key=lambda x: x[0])
                            else:
                                chosen = max(selectable, key=lambda x: x[0])
        ...
        if round == 6:
            if len(self.player.hand) <= 3 and len(self.player.table) == 3 and \
                    max([card for card in self.player.table
                         if card[1] == self.player.table[0][1]]) == self.player.table[-2]:
