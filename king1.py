...
class Agent:
    ...
    def play(self, table):
        ...
        if self.round != 2 and self.round != 5:
            ...
            # In round 5, hearts can only be played first when the player has no other suit, and the king of hearts
            # must be played at first chance
            if not table:
                not_hearts = [card for card in self.hand if card[1] != 'H']
                if not not_hearts:
                    selectable = self.hand
...
def round1(strat0, strat1, strat2, strat3):
    ...
    for i in range(13):
        ...
        points[winner] -= 20
        log += 'round: ' + str(i + 1) + '\n'
        log += 'table: [a' + str(first) + ': ' + str(table[0]) + ', a' + str((first + 1) % 4) + ': ' + \
               str(table[1]) + ', a' + str((first + 2) % 4) + ': ' + str(table[2]) + ', a' + str((first + 3) % 4) + \
               ': ' + str(table[3]) + ']\n'
...
def round2(strat0, strat1, strat2, strat3):
    ...
    for i in range(13):
        ...
        log += 'round: ' + str(i + 1) + '\n'
        log += 'table: [a' + str(first) + ': ' + str(table[0]) + ', a' + str((first + 1) % 4) + ': ' + str(
            table[1]) + ', a' + str((first + 2) % 4) + ': ' + str(table[2]) + ', a' + str((first + 3) % 4) + ': ' + str(
            table[3]) + ']\n'
        log += 'winner: ' + str(winner) + '\n'
        log += 'score: ' + str(points) + '\n \n'
        first = winner
...
round2("random", "random", "random", "random")
round3("random", "random", "random", "random")
round4("random", "random", "random", "random")
round5("random", "random", "random", "random")
round6("random", "random", "random", "random")
